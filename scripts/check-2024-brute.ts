...
async function main() {
    ...
    // Group by issueType to see what's being excluded
    ...
    // Check if there are any tickets with 2024 in the "year" field but different createdDate
    const byYearField = await prisma.ticket.count({ where: { year: 2024 } });
    console.log('Tickets with year=2024 field:', byYearField);
}
