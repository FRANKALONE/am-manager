...
export async function syncHistoryInBulk(issueKeys: string[], type: 'TICKET' | 'PROPOSAL') {
    ...
    // To ensure full changelog for EVERY issue, we iterate and call syncTicketHistory
    // This is safer than the expanded search which truncates history.
    // We can use a small limit on concurrency if needed, but for batches of 50 it's fine.
    for (const key of issueKeys) {
        await syncTicketHistory(key, type);
    }
}
