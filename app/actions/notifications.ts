        ...
        if (count === 0) {
            await prisma.notificationSetting.createMany({
                data: [
                    ...
                    { type: 'CONTRACT_RENEWED', title: 'Contrato Renovado', description: 'Notifica cuando un contrato ha sido renovado.', group: 'CONTRACTS', roles: 'ADMIN,GERENTE,COLABORADOR', appMessage: '✅ Renovación Automática: {clientName} - {wpName}', emailSubject: '✅ Renovación Automática: {clientName} - {wpName}', emailMessage: 'Se ha renovado automáticamente el WP "{wpName}" hasta el {endDate} con un incremento del {ipcValue}% (Tarifa: {newRate}€).' },
                    ...
                    { type: 'PROPOSAL_REQUESTED', title: 'Propuesta Solicitada', description: 'Notifica cuando un cliente solicita una propuesta de optimización.', group: 'SYSTEM', roles: 'ADMIN,GERENTE', appMessage: '🚀 El cliente {clientName} ha solicitado una propuesta sobre {topic}.', emailSubject: 'Propuesta Solicitada: {clientName}', emailMessage: 'El cliente {clientName} está interesado en una propuesta de optimización relacionada con {topic}.\n\nDetalle compartido: {details}' }
                ]
            });
        }
