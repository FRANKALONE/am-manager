...
console.log('Searching for date and status related fields...');
...
https.get(url, options, (res) => {
    ...
    res.on('end', () => {
        try {
            ...
            const found = fields.filter(f => f.name && (
                f.name.toLowerCase().includes('fecha') ||
                f.name.toLowerCase().includes('date') ||
                f.name.toLowerCase().includes('entrega') ||
                f.name.toLowerCase().includes('enviado') ||
