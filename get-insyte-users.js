...
async function main() {
    try {
        console.log('═══════════════════════════════════════════════════════════════');
        console.log('  Usuarios de Cliente - INSYTE INSTALACIONES (INS)');
        console.log('═══════════════════════════════════════════════════════════════\n');
        ...
        // Buscar el Service Desk con projectKey = INS
        const insyteServiceDesk = serviceDesks.find(sd => sd.projectKey === 'INS');

        if (!insyteServiceDesk) {
            console.error('❌ No se encontró Service Desk con clave "INS"');
            console.log('\n📋 Service Desks disponibles:');
            serviceDesks.forEach(sd => console.log(`   - ${sd.projectName} (${sd.projectKey})`));
            return;
        }

        console.log(`✅ Service Desk encontrado: ${insyteServiceDesk.projectName}`);
        console.log(`   Project Key: ${insyteServiceDesk.projectKey}`);
        console.log(`   Service Desk ID: ${insyteServiceDesk.id}`);

        // Obtener organizaciones
        console.log('\n🏢 Obteniendo organizaciones...');
        const orgsResponse = await makeRequest(`/servicedesk/${insyteServiceDesk.id}/organization`, true);
        ...
        console.log('─'.repeat(70));
        console.log(`\n📊 RESUMEN:`);
        console.log(`   • Proyecto: ${insyteServiceDesk.projectName} (${insyteServiceDesk.projectKey})`);
